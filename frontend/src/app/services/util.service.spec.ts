...
environments: !!map  # specific environments can override the default values 1 property at a time
  ...
  it('should convert between Yaml and TreeNode', () => {
    ...
    const tree = utilService.convertYamlToTree(yaml, false);
  ...
  it('array with same simple type should be supported', () => {
    ...
    const tree: TreeNode = utilService.convertYamlToTree(yaml, true);
  ...
  it('should compile yaml', () => {
    ...
    config.environments.findChild(['dev', 'arr1']).addChild(new TreeNode('[0]', PROPERTY_VALUE_TYPES.STRING, 'dev-item1-value', ['dev-item1-comment']));
