...
describe("hydrate", () => {
    ...
    describe("hydrateFile", () => {
        ...
        test("Success", async () => {
            ...
            for (const env of envs) {
                ...
                const expectedJsonPath = path.join(__dirname, "data/expectedResults", env, "app.config.json");
        ...
        test("YAML with anchors and aliases", async () => {
            ...
            for (const env of envs) {
                ...
                const expectedJsonPath = path.join(
                    __dirname,
                    "data/anchoredConfigExpectedResults",
                    env,
                    "app.config.json",
                );
                const expectedJson = await fs.readJson(expectedJsonPath);
        ...
        test("No default block", async () => {
            const inputFile = path.join(__dirname, "data/appWithInvalidConfig/no.default.yaml");
        ...
        test("No environments block", async () => {
            const inputFile = path.join(__dirname, "data/appWithInvalidConfig/no.environments.yaml");
            const outputFile = path.join(outputFolder, "/appWithInvalidConfig");
            await expect(hydrate.hydrateFile(inputFile, undefined, undefined, outputFile))
                .rejects.toMatchObject({message: `Invalid config file format (${inputFile})`});
        });
        ...
        test("Cyclic Inherits", async () => {
            const inputFile = path.join(__dirname, "data/appWithCyclicInherits/app.config.yaml");
        ...
        test("Token Cyclic Reference", async () => {
            const inputFile = path.join(__dirname, "data/appWithTokenCyclicReference/app.config.yaml");
            const outputFile = path.join(outputFolder, "/appWithTokenCyclicReference");
            await expect(hydrate.hydrateFile(inputFile, undefined, undefined, outputFile))
                .rejects.toMatchObject(/Cyclic variable reference detected/);
        });

        test("Token Cyclic Self Reference", async () => {
            const inputFile = path.join(__dirname, "data/appWithTokenCyclicReference/self-reference.yaml");
            const outputFile = path.join(outputFolder, "/appWithTokenCyclicReference");
            await expect(hydrate.hydrateFile(inputFile, undefined, undefined, outputFile))
        ...
        test("With VariableNamePrefix key", async () => {
            const inputFile = path.join(__dirname, "data/appWithVariableNamePrefixKey/app.config.yaml");
            const outputFile = path.join(outputFolder, "/appWithVariableNamePrefixKey");
            await expect(hydrate.hydrateFile(inputFile, undefined, undefined, outputFile))
                .resolves.toBeUndefined();
            const envs = ["dev", "local", "prod", "qat"];
            await expect(utils.findSubFolders(outputFile)).resolves.toEqual(envs);
        ...
        test("Config with array", async () => {
            const inputFile = path.join(__dirname, "data/appWithTokenCyclicReference/array.config.yaml");
            const outputFile = path.join(outputFolder, "/appWithTokenCyclicReference");
            await expect(hydrate.hydrateFile(inputFile, undefined, undefined, outputFile))
                .resolves.toBeUndefined();
            const envs = ["dev", "local", "prod", "qat"];
    ...
    describe("hydrateApp", () => {
        ...
        test("Success", async () => {
            ...
            const outputFile = path.join(outputFolder, "/shop");
            await expect(hydrate.hydrateApp(inputFile, undefined, outputFile))
                .resolves.toBeUndefined();
        });
        test("No environments file", async () => {
            ...
            await expect(hydrate.hydrateApp(inputFile, undefined, outputFile))
                .rejects.toMatchObject({message: `Environment file '${envFilePath}' doesn't exist`});
        });
    });
    ...
    describe("hydrateAllApps", () => {
        ...
        test("Invalid input folder", async () => {
            await expect(hydrate.hydrateAllApps("wrong-input", outputFolder))
                .rejects.toMatchObject({message: expect.stringContaining(
                    "ENOENT: no such file or directory, scandir")});
        });
        test("Success", async () => {
            await expect(hydrate.hydrateAllApps(inputFolder, outputFolder)).resolves.toBeUndefined();
        });
    });
