...
export class FileManagementService {
  ...
  async getFileContent(principal: Principal, file: ConfigFile): Promise<ConfigFile> {
    ...
    if (await this.isRepoFileExists(pathFinder)) {
      ...
      const { object, oid } = await git.readObject(
        { dir: pathFinder.repoDir, oid: commitOid, filepath: pathFinder.repoFilePath, encoding: 'utf8' });
      file.oid = oid;
      ...
      if (!forcePush) {
        ...
        if ((!oldOid && newOid) || (oldOid && newOid && oldOid !== newOid)) { // Should conflict if found to be deleted?
          const { object } = await git.readObject(
            { dir: pathFinder.repoDir, oid: pulledCommit, filepath: pathFinder.repoFilePath, encoding: 'utf8' });
